    ...
    public int[] solution(int[] fees, String[] records) {
        ...
        for(int i = 0; i < records.length; i++){
            ...
            if(map.containsKey(infos[1])){
                ...
                feeMap.replace(infos[1], 
                    feeMap.get(infos[1]) + 60 * hour + minute);
            } else { 
        ...
        Collections.sort(list, (o1, o2) -> {
            return Integer.parseInt(o1.getKey()) > Integer.parseInt(o2.getKey()) ? 1 
                 : Integer.parseInt(o1.getKey()) < Integer.parseInt(o2.getKey()) ? -1 
                 : 0;
        });
        ...
        for(int i = 0; i < answer.length; i++){
            if(list.get(i).getValue() > fees[0]){
                answer[i] = fees[1] + 
                    (int) Math.ceil((list.get(i).getValue() - fees[0]) / (double)fees[2]) * fees[3];
            } else {
                answer[i] = fees[1];
            }
        }
